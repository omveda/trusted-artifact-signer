...
openssl ec -in file_ca_key.pem -passin pass:haha123 -pubout -out file_ca_pub.pem
openssl req -new -x509 -days 365 -key file_ca_key.pem -passin pass:haha123  -out fulcio-root.pem -passout pass:"$password" -subj "/CN=$common_name/emailAddress=$email_address/O=$organization_name"
...
helm repo update
OPENSHIFT_APPS_SUBDOMAIN=$common_name envsubst < examples/values-sigstore-openshift.yaml | helm install --debug trusted-artifact-signer trusted-artifact-signer/trusted-artifact-signer -n trusted-artifact-signer --values -
